<script lang="ts">
  ...
  async function handleSignup() {
    ...
    try {
      ...
      if (res.ok) {
        if (data.demoMode) {
          // Demo mode - account created without email verification
          // Redirect to login
...
<svelte:head>
  <title>Sign Up | Temporal Cloud</title>
</svelte:head>
    ...
    {#if success}
      <div class="text-center space-y-4">
        ...
        <p class="text-zinc-400">
          We've sent a verification link to <span class="text-emerald-400">{email}</span>. 
          Click the link in the email to verify your account.
        </p>
        <p class="text-sm text-zinc-500">
          Didn't receive the email? Check your spam folder or 
          <button on:click={() => success = false} class="text-emerald-400 hover:text-emerald-300">try again</button>.
        </p>
        <a href="/login" class="inline-block mt-4 text-sm text-zinc-400 hover:text-white">
          ← Back to sign in
        </a>
      </div>
    {:else}
      <div class="text-center">
        ...
        <p class="mt-2 text-sm text-zinc-400">Start building durable applications</p>
      </div>
      ...
      <div class="relative">
        ...
        <div class="relative flex justify-center text-sm">
          <span class="px-2 text-zinc-500 bg-[#0d0620]">or sign up with email</span>
        </div>
      </div>
      ...
      <p class="text-center text-sm text-zinc-500">
        Already have an account? <a href="/login" class="text-emerald-400 hover:text-emerald-300">Sign in</a>
      </p>

      <p class="text-center text-xs text-zinc-600">
        By signing up, you agree to our 
        <a href="/terms" class="text-zinc-500 hover:text-zinc-400">Terms of Service</a> and 
        <a href="/privacy" class="text-zinc-500 hover:text-zinc-400">Privacy Policy</a>.
      </p>
    {/if}
  </div>
</div>
